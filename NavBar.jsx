    ...
    <div className='header-menu-container'>
      ...
      <div className={`destination-search ${showDestinationSearch ? 'show' : ''}`} ref={destinationSearchRef}>
        <div className="navbar">
          <button className={`navbar-item ${showRegionGrid ? 'active' : ''} ${selectedDestination !== 'Search Destination' ? 'item-selected' : ''}`} onClick={handleWhereButtonClick}>
          ...
          <button className={`navbar-item ${showGuestOptions ? 'active' : ''} ${totalGuests > 0 ? 'item-selected' : ''}`}  onClick={toggleGuestOptions}>
            Who<br /><span style={{ color: 'slategray' }}>{totalGuests > 0 ? formatGuestCount() : 'Add guests'}</span>
          </button>
          <button className="search-button">Search</button>
          {showGuestOptions && (
            <div className="guest-selection">
              <div className="guest-item">
                <span>Adults<br/><small>Ages 13 or above</small></span>
               
                <div className="counter">
                  <button onClick={() => decrementCount(setAdults, adults)} disabled={adults === 0}>-</button>
                  <span>{adults}</span>
                  <button onClick={() => incrementCount(setAdults, adults)}>+</button>
                </div>
              </div>
              <div className="guest-item">
                ...
                <div className="counter">
                  <button onClick={() => decrementCount(setChildren, children)} disabled={children === 0}>-</button>
                  <span>{children}</span>
                  <button onClick={() => incrementCount(setChildren, children)}>+</button>
                </div>
              </div>
              <div className="guest-item">
                <span>Infants<br/><small>Under 2</small></span>
                
                <div className="counter">
                  <button onClick={() => decrementCount(setInfants, infants)} disabled={infants === 0}>-</button>
                  <span>{infants}</span>
                  <button onClick={() => incrementCount(setInfants, infants)}>+</button>
                </div>
              </div>
              <div className="guest-item">
                <span>Pets<br/><small>Bringing a service a animal?</small></span>
                <div className="counter">
                  <button onClick={() => decrementCount(setPets, pets)} disabled={pets === 0}>-</button>
                  <span>{pets}</span>
                  <button onClick={() => incrementCount(setPets, pets)}>+</button>
                </div>
              </div>
            </div>
          )}
